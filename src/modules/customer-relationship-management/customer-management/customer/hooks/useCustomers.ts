...
export function useCustomers(): UseCustomersReturn {
    ...
    async (showLoading = false) => {
      try {
        ...
        const [customerRes, userRes] = await Promise.all([
          fetch(`/api/crm/customer?${params.toString()}`, {
            cache: "no-store",
          }),
        ...
        if (userRes.ok) {
          ...
          (userData.data || []).forEach((u: ReferenceItem) => {
            const fullName = [u.user_fname, u.user_mname, u.user_lname]
              .filter(Boolean)
              .join(" ");
  ...
  useEffect(() => {
    const t = setTimeout(() => fetchData(true), 0);
    return () => clearTimeout(t);
  }, [fetchData]);
    ...
    async (data: Partial<CustomerWithRelations>) => {
      try {
        ...
        if (bank_accounts && bank_accounts.length > 0) {
          await Promise.all(
            bank_accounts.map(async (account) => {
              // Strip ID and metadata for new records to prevent Directus errors
              // eslint-disable-next-line @typescript-eslint/no-unused-vars
              const { id, created_at, updated_at, ...cleanedAccount } =
      ...
      try {
        ...
        if (newAccounts) {
          ...
          const currentRes = await fetch(
            `/api/crm/customer/bank-account?customer_id=${id}`,
          );
          const oldAccounts: BankAccount[] = currentRes.ok
            ? await currentRes.json()
            : [];

          // Identify Deletions
          const toDelete = oldAccounts.filter(
            (old) => !newAccounts.some((n) => n.id === old.id),
          );
          ...
          const toPost = newAccounts.filter((n) => {
            ...
            const alreadyExists = oldAccounts.some(
              (old) => old.account_number === n.account_number,
            );
            return isNew && !alreadyExists;
          });

          const toPatch = newAccounts
            .filter((n) => {
              const hasId = n.id && n.id !== 0;
              const matchesOld = oldAccounts.some(
                (old) => old.account_number === n.account_number,
              );
            ...
            .map((n) => {
              ...
              if (!n.id || n.id === 0) {
                const oldMatch = oldAccounts.find(
                  (old) => old.account_number === n.account_number,
                );
          ...
          for (const acc of toDelete) {
            const delRes = await fetch(
              `/api/crm/customer/bank-account?id=${acc.id}`,
              { method: "DELETE", cache: "no-store" },
            );
            if (!delRes.ok)
              throw new Error(
                `Failed to delete bank account: ${delRes.statusText}`,
              );
          }
          ...
          for (const acc of toPost) {
            ...
            if (!postRes.ok) {
              const errTxt = await postRes
                .text()
                .catch(() => postRes.statusText);
          ...
          for (const acc of toPatch) {
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const { created_at, updated_at, ...cleanedAccount } =
              acc as BankAccount;
            ...
            if (!patchRes.ok) {
              const errTxt = await patchRes
                .text()
                .catch(() => patchRes.statusText);
