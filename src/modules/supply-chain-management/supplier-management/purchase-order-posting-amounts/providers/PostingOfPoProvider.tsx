...
const API = "/api/scm/supplier-management/purchase-order-posting-amounts";
...
export function PostingOfPoProvider({ children }: { children: React.ReactNode }) {
    ...
    React.useEffect(() => {
        ...
        fetch("/api/scm/supplier-management/purchase-order-posting-amounts/discount-types")
            .then(r => r.json())
            .then(j => {
    ...
    const openPO = React.useCallback(
        async (poId: string) => {
            ...
            try {
                ...
                // ✅ Handle Locked/Posted status by refreshing
