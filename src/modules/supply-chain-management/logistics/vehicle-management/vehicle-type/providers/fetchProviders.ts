import { VehicleTypeApiRow, VehicleTypeFormValues } from "../types";
...
const API_BASE = "/api/scm/logistics/vehicle-management/vehicle-type";
...
export async function listVehicleTypes(
  ...
  limit = 12,
  search = "",
): Promise<{ data: VehicleTypeApiRow[]; total: number }> {
...
export async function createVehicleType(
  payload: VehicleTypeFormValues,
): Promise<VehicleTypeApiRow> {
  ...
  if (!res.ok) throw new Error(await readError(res));
  const json = (await res.json()) as DirectusItemResponse<VehicleTypeApiRow>;
  return json?.data;
}
