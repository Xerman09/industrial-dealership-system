...
interface VATSaleLineChartProps {
  ...
  /** Pass true when a date/customer/supplier filter is active */
  isFiltered?: boolean;
}

export function VATSaleLineChart({ data, isFiltered = false }: VATSaleLineChartProps) {
  // Determine primary year from data
  ...
  if (!isFiltered) {
    // Default: all 12 months scaffolded, fill with actual values
    aggregated = MONTH_KEYS.map((m) => ({
      month: `${m} ${primaryYear}`,
      amount: monthMap[`${m} ${primaryYear}`] ?? 0,
    }));
  } else {
    // Filtered: only months with data
    ...
    <Card>
      ...
      <CardFooter className="flex-col items-start gap-2 text-sm">
        ...
        <div className="leading-none text-muted-foreground">
          Showing monthly VAT sales totals
        </div>
      </CardFooter>
    </Card>
  );
