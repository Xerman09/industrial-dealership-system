import { formatDateTime, formatPHP, toNumberSafe } from "../../for-receiving/utils/format";
...
export function printTransmittalHistory(args: {
    header: Header;
    lines: Line[];
    ...
    const rowsHtml =
        groups.length === 0
            ? `<tr><td colspan="7" class="muted" style="text-align:center;padding:16px;">No records.</td></tr>`
            : groups
                .map((g) => {
                    const headerRow = `
  ...
  <td colspan="7">
    <div class="groupTitle">${esc(g.label)}</div>
                    ...
                    const itemRows = g.rows
                        .map((r) => {
                            const transactionDate = r.credit_date ?? r.created_at ?? null;
                            const cmNo = safeText(r.memo_number) || "—";
                            const rowSupplier = safeText(r.supplier_name) || supplier || "—";
                            const customer = safeText(r.customer_name) || "—";
                            const coa = coaLabelFor(r);
                            const remarks = safeText(r.remarks) || safeText(r.reason) || "—";
                            const amount = formatPHP(toNumberSafe(r.total ?? r.amount));

                            return `
<tr class="avoidBreak">
  <td>${esc(coa)}</td>
  <td>${esc(customer)}</td>
  <td>${esc(rowSupplier)}</td>
  <td class="mono">${esc(cmNo)}</td>
  <td class="nowrap dateCell">${esc(transactionDate ? formatDateTime(transactionDate) : "—")}</td>
  <td>${esc(remarks)}</td>
  <td class="num nowrap">${esc(amount)}</td>
</tr>`;
                        })
                        .join("");
...
<tr class="subRow avoidBreak">
  <td colspan="6" class="subLabel">Subtotal — ${esc(g.label)}</td>
...
<title>Transmittal History - ${esc(transNo)}</title>
<style>
  ...
  .groupTitle { font-weight: 900; color:#111; }
  ...
  <div class="page">
    <div class="content">
      <div class="companyBar">
        ...
        <div class="docInfo">
          <div class="docTitle">Claims Transmittal History</div>
      ...
      <table>
        <thead>
          <tr>
            <th style="width: 16%;">COA NAME</th>
            <th style="width: 16%;">Customer</th>
            <th style="width: 14%;">Supplier</th>
            <th style="width: 12%;">CM No</th>
            <th style="width: 13%;">Transaction Date</th>
            <th style="width: 17%;">Description of Claims or Remarks</th>
            <th style="width: 12%; text-align:right;">Amount Claimed</th>
          </tr>
        </thead>
    ...
    <div class="footer avoidBreak">
      ...
      <div class="totals">
        ...
        <div class="value">${esc(formatPHP(grandTotal))}</div>
      </div>
    </div>
  </div>
