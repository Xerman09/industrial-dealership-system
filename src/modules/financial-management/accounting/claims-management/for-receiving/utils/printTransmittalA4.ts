// src/modules/financial-management/claims-management/for-receiving/utils/printTransmittalA4.ts
...
export function printTransmittalA4(args: {
    header: Header;
    lines: Line[];
    ...
    const status = safeText(header.status) || "FOR RECEIVING";
    ...
    const rowsHtml =
        groups.length === 0
            ? `<tr><td colspan="7" class="muted" style="text-align:center;padding:16px;">No CCM lines.</td></tr>`
            : groups
                .map((g) => {
                    const headerRow = `
    ...
    <div class="groupTitle">${esc(g.label)}</div>
                    ...
                    const itemRows = g.rows
                        .map((r) => {
                            ...
                            const total = formatPHP(toNumberSafe(r.total ?? r.amount));

                            return `
<tr class="avoidBreak">
  <td>${esc(coa)}</td>
  <td class="nowrap dateCell">${esc(creditDate ? formatDateTime(creditDate) : "—")}</td>
  ...
  <div class="page">
    <div class="content">
      ...
      <table>
        <thead>
          <tr>
            <th style="width: 16%;">Chart of Accounts</th>
            <th style="width: 11%;">Credit Date</th>
            <th style="width: 14%;">Credit Memo No.</th>
