// src/modules/financial-management/accounting/supplier-credit-memo/components/MemoTable.tsx
...
import type { SupplierCreditMemo, Supplier, ChartOfAccount } from "../types";
  ...
  return (
    <Card className="shadow-none border-border overflow-hidden">
      <CardHeader className="bg-muted/30 border-b border-border/50 flex flex-row items-center justify-between gap-4">
        <CardTitle className="text-sm font-bold uppercase shrink-0">SCM Records</CardTitle>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={7} className="text-center py-10 text-muted-foreground text-sm">
                  {q ? `No results for "${search}".` : "No credit memos found."}
                </TableCell>
              </TableRow>
            ) : (
              paged.map((m, i) => (
