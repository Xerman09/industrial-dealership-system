...
import { IndustryFilterProvider } from "./providers/IndustryFilterProvider";
...
function IndustryModuleContent() {
    const { 
        industries, 
        isLoading, 
        isError,
    ...
    const handleUpdate = async (id: number, data: IndustryFormData) => {
        try {
            ...
            <Alert variant="destructive" className="rounded-xl border-2 shadow-lg">
                ...
                <AlertDescription className="mt-2 flex flex-col gap-4">
                    <p className="text-sm opacity-90 leading-relaxed">
                        ...
                        Error: <span className="font-mono font-bold bg-destructive-foreground/10 px-1 rounded">{error instanceof Error ? error.message : "Unknown error"}</span>
                    </p>
                    <Button
                        variant="outline"
                        size="sm"
                ...
                <CardHeader className="px-0 pt-0 flex flex-row items-end justify-between gap-4 pb-0">
                    <div className="space-y-1.5 min-w-0">
                        ...
                        <CardDescription className="text-base text-muted-foreground font-medium leading-relaxed max-w-2xl truncate">
                            Manage and organize all industrial sectors with precision and ease.
                        </CardDescription>
                    </div>
...
export function IndustryModule() {
    return (
        <IndustryFilterProvider>
            <IndustryModuleContent />
        </IndustryFilterProvider>
    );
}
