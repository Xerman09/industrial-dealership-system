...
export async function GET(request: NextRequest) {
  ...
  const targetUrl = `${base}/api/view-fm-report-vat-purchasing/all${query.toString() ? `?${query}` : ''}`;
  ...
  try {
    ...
    if (!springRes.ok) {
      console.error('[VAT-PURCHASES] Upstream error:', springRes.status, springRes.statusText);
