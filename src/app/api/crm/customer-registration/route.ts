...
async function fetchClassificationName(
  ...
  return json?.data?.classification_name ? String(json.data.classification_name) : null;
}
  ...
  try {
    ...
    if (id) {
      const [customerRes, bankRes] = await Promise.all([
        fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.CUSTOMER}/${id}`, { cache: "no-store", headers }),
        fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.BANK_ACCOUNTS}?filter[customer_id][_eq]=${id}`, { cache: "no-store", headers }),
      ]);
      ...
      return NextResponse.json({ ...customerData.data, bank_accounts: bankData.data || [] });
    }
    ...
    if (!isUnlimited) {
        ...
        if (statusFilter !== "all") params.append("filter[isActive][_eq]", statusFilter === "active" ? "1" : "0");
    ...
    const customersRes = await fetchWithRetry(customersUrl, { cache: "no-store", headers });
    ...
    if (customerIds.length > 0) {
      ...
      const [bankRes, csRes] = await Promise.all([
        fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.BANK_ACCOUNTS}?filter[customer_id][_in]=${idsQuery}`, { cache: "no-store", headers }),
        fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.CUSTOMER_SALESMEN}?filter[customer_id][_in]=${idsQuery}`, { cache: "no-store", headers })
      ]);

      if (bankRes.ok) bankAccounts = (await bankRes.json()).data || [];
      if (csRes.ok) {
        customerSalesmen = (await csRes.json()).data || [];
        const salesmanIds = Array.from(new Set(customerSalesmen.map(cs => cs.salesman_id).filter(Boolean)));
        
        if (salesmanIds.length > 0) {
          const salesmanRes = await fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.SALESMAN}?filter[id][_in]=${salesmanIds.join(",")}`, { cache: "no-store", headers });
          if (salesmanRes.ok) salesmen = (await salesmanRes.json()).data || [];
        }
      }
    }

    const enrichedCustomers = customers.map((customer: any) => {
      const myBanks = bankAccounts.filter((acc: any) => String(acc.customer_id) === String(customer.id));
      const mySalesmenLinks = customerSalesmen.filter((cs: any) => String(cs.customer_id) === String(customer.id));
      ...
      if (firstLink) {
        const salesmanData = salesmen.find((s: any) => String(s.id) === String(firstLink.salesman_id));
        if (salesmanData) {
      ...
      if (mappedLocation && typeof mappedLocation === "object" && "coordinates" in mappedLocation && Array.isArray((mappedLocation as any).coordinates)) {
        const coords = (mappedLocation as any).coordinates;
    ...
    return NextResponse.json({ error: "Failed to fetch customers", message: e instanceof Error ? e.message : "Unknown error" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (newCustomerData.status === undefined && newCustomerData.profile_status !== undefined) {
    ...
    if (newCustomerData.location !== undefined) {
      ...
      newCustomerData.location = geoJson ? geoJson : (newCustomerData.location === "" || newCustomerData.location === null) ? null : newCustomerData.location;
    }

    const isWalkIn = await isWalkInClassification(newCustomerData.classification, token);
    const normalizedTin = normalizeTin(newCustomerData.customer_tin);

    if (!isWalkIn) {
      if (!normalizedTin) return NextResponse.json({ error: "TIN is required unless classification is Walk-in" }, { status: 400 });
      if (!String(newCustomerData.province ?? "").trim() || !String(newCustomerData.city ?? "").trim() || !String(newCustomerData.brgy ?? "").trim()) {
        return NextResponse.json({ error: "Delivery address is required unless classification is Walk-in" }, { status: 400 });
      }

      const dup = await findCustomerByTin(normalizedTin, token);
      if (dup.length > 0) return NextResponse.json({ error: "TIN must be unique" }, { status: 409 });
      newCustomerData.customer_tin = normalizedTin;
    } else {
      newCustomerData.customer_tin = normalizeEmptyToNull(newCustomerData.customer_tin);
    ...
    const createRes = await fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.CUSTOMER}`, { method: "POST", headers, body: JSON.stringify(newCustomerData) });
    ...
    const patchRes = await fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.CUSTOMER}/${newId}`, { method: "PATCH", headers, body: JSON.stringify({ customer_code: generatedCode }) });
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (updateData.status === undefined && updateData.profile_status !== undefined) {
      updateData.status = updateData.profile_status;
    }
    delete updateData.profile_status;

    if (!updateData.customer_code || updateData.customer_code.trim() === "") delete updateData.customer_code;
    ...
    const shouldValidate = updateData.customer_tin !== undefined || updateData.classification !== undefined || updateData.province !== undefined;
    if (shouldValidate) {
      const existingRes = await fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.CUSTOMER}/${id}`, { cache: "no-store", headers });
      const existing = existingRes.ok ? (await existingRes.json()).data : null;
      
      const classificationId = updateData.classification ?? existing?.classification ?? null;
      const isWalkIn = await isWalkInClassification(classificationId, token);
      const resolvedTin = updateData.customer_tin !== undefined ? normalizeTin(updateData.customer_tin) : normalizeTin(existing?.customer_tin);

      if (!isWalkIn) {
        if (!resolvedTin) return NextResponse.json({ error: "TIN is required unless classification is Walk-in" }, { status: 400 });
        const dup = await findCustomerByTin(resolvedTin, token, id);
        if (dup.length > 0) return NextResponse.json({ error: "TIN must be unique" }, { status: 409 });
        updateData.customer_tin = resolvedTin;
      } else {
        if (updateData.customer_tin !== undefined) updateData.customer_tin = normalizeEmptyToNull(updateData.customer_tin);
        if (updateData.province !== undefined) updateData.province = normalizeEmptyToNull(updateData.province);
        if (updateData.city !== undefined) updateData.city = normalizeEmptyToNull(updateData.city);
        if (updateData.brgy !== undefined) updateData.brgy = normalizeEmptyToNull(updateData.brgy);
      }
    }

    const res = await fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.CUSTOMER}/${id}`, { method: "PATCH", headers, body: JSON.stringify(updateData) });
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (!id) return NextResponse.json({ error: "Customer ID is required" }, { status: 400 });
    const res = await fetchWithRetry(`${DIRECTUS_URL}/items/${COLLECTIONS.CUSTOMER}/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
