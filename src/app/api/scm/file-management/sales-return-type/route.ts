...
const ENDPOINT = "/items/sales_return_type";
...
// Fetch user records from the user table to resolve IDs → names
async function resolveUserNames(
  ...
  try {
    ...
    // 2. If no rows found, or if some IDs are still missing, try native /users
    const foundIds = new Set(rows.map(r => Number(r.user_id)));
    ...
    if (missingIds.length > 0) {
      ...
      if (fallbackRes.ok) {
        ...
        // Normalize fallback rows to match the loop below
  ...
  try {
    ...
    if (method === "GET" && !id && Array.isArray(dataResponse?.data)) {
      ...
      dataResponse.data = rows.map((row) => {
        const cb = row.created_by ?? row.user_created ?? row.creator;
        const ub = row.updated_by ?? row.user_updated ?? row.modifier;
