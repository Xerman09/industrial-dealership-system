...
export async function GET() {
  try {
    ...
    // Fetch undertime requests based on department
    // If user is from HR (department_id = 2), fetch all requests
    // Otherwise, fetch only their department's requests
    let undertimeUrl = `/items/undertime_request?limit=1000&sort=-filed_at&fields=*`;

    if (userDepartment !== 2) {
      undertimeUrl += `&filter[department_id][_eq]=${userDepartment}`;
    }
    ...
    const userIds = [...new Set(requests.map((r: UndertimeRequest) => r.user_id))].filter(
    ...
    // Enrich undertime requests with user details
